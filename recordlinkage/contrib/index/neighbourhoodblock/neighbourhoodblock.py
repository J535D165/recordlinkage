...
class NeighbourhoodBlock(Block):
    ...
    **kwargs :
    ...
    def __init__(self,
                 ...
                 windows=1,
                 **kwargs):
        super(NeighbourhoodBlock, self).__init__(left_on=left_on,
                                                 right_on=right_on,
                                                 **kwargs)
    ...
    def _index(self, df_a, df_b=None):
        ...
        def deduped_blocks_and_indices(blocks, indices=None):
            ...
            deduped_blocks, index_tx = numpy.unique(blocks,
                                                    axis=0,
                                                    return_inverse=True)
        ...
        def get_normalized_linkage_params():
            ...
            windows = self.windows + self.windows[-1:] * (n_keys -
                                                          len(self.windows))
        ...
        def linkage_index_codes(blocks,
            ...
            if any(len(x) <= 1 for x in indices) or (
                    pandas.Series(rank_max -
                                  rank_distance_limits.flatten()).max()
                    in [0, numpy.nan]):
                block_pair_candidates = numpy.vstack(
                    [a.flatten() for a in numpy.meshgrid(*indices)]).T
            else:
            ...
            if len(block_pair_candidates) > 0:
                block_pair_candidates = numpy.unique(block_pair_candidates,
                                                     axis=0)
            excess_rank_distances = numpy.abs(
                blocks[block_pair_candidates.T[0]] -
                blocks[block_pair_candidates.T[1]]) - rank_distance_limits
            null_counts = numpy.sum(numpy.isnan(excess_rank_distances), axis=1)
            match_counts = numpy.sum(
                numpy.nan_to_num(excess_rank_distances) <= 0.5,
                axis=1) - null_counts
            block_pair_accepted = (match_counts + numpy.clip(
                null_counts, 0, self.max_nulls)) >= (blocks.shape[1] -
                                                     self.max_non_matches)
        ...
        if any(len(df) == 0 for df in dfs):
            ...
            blocks, indices, rank_distance_limits = \
                get_normalized_linkage_params()
            rownum_pairs = linkage_index_codes(blocks, indices,
                                               rank_distance_limits)
        if df_b is None:  # dedup index
            rownum_pairs = rownum_pairs[rownum_pairs.T[0] > rownum_pairs.T[1]]
        index = pandas.MultiIndex(levels=[df.index.values for df in dfs],
