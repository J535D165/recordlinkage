    ...
    **kwargs :
    ...
    def _index(self, df_a, df_b=None):
        ...
        def linkage_index_codes(blocks,
            ...
            if any(len(x) <= 1 for x in indices) or (pandas.Series(
                    rank_max - rank_distance_limits.flatten()).max() in [
                        0, numpy.nan]):
            ...
            if len(block_pair_candidates) > 0:
                block_pair_candidates = numpy.unique(
                    block_pair_candidates, axis=0)
