...
def simulate_features(n=10000, p=0.2, m=None, u=None, n_features=8):
    ...
    # Create data
    data_matches = numpy.array([simulate_similarity(
        n_matches, mi[0], mi[1]) for mi in list(m)]).T
    data_nonmatches = numpy.array([simulate_similarity(
        n_nonmatches, ui[0], ui[1]) for ui in list(u)]).T
    ...
    index = numpy.random.choice(numpy.arange(
        1e5, 1e5 + 2 * n), size=(2, n), replace=False).astype(int)
...
def addmissingvalues(df, missing_dict=MISSING_DICT):
    ...
    for col in list(df):
        ...
        if col in missing_dict.keys():
            ...
            to_replace = numpy.random.choice([True, False], len(
                df), p=[missing_dict[col], 1 - missing_dict[col]])
...
def addsubstitutions(df, subs_dict=SUBS_DICT):
    ...
    for col in list(df):
        ...
        if col in subs_dict.keys():
            ...
            # Make a random sample of values to replace.
            to_replace = numpy.random.choice([True, False], len(
                df), p=[subs_dict[col], 1 - subs_dict[col]])

            # Special case when for first name
            if col == 'first_name':

                if 'sex' == 'M':
                    df.loc[to_replace, col] = [fake.first_name_male()
                                               for _ in range(0, sum(to_replace))]
                elif 'sex' == 'F':
                    df.loc[to_replace, col] = [fake.first_name_female()
                                               for _ in range(0, sum(to_replace))]
                else:
                    pass
            elif col == 'last_name':
                df.loc[to_replace, col] = [fake.last_name()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'phone_number':
                df.loc[to_replace, col] = [fake.phone_number()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'job':
                df.loc[to_replace, col] = [fake.job()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'email':
                df.loc[to_replace, col] = [fake.free_email()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'birthdate':
                df.loc[to_replace, col] = [fake.date()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'street_address':
                df.loc[to_replace, col] = [fake.street_address()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'postcode':
                df.loc[to_replace, col] = [fake.postcode()
                                           for _ in range(0, sum(to_replace))]
            elif col == 'city':
                df.loc[to_replace, col] = [fake.city()
                                           for _ in range(0, sum(to_replace))]
...
def dataset(N, df=None, matches=None):
    ...
    if df is None:
        ...
        df_persons.set_index(numpy.arange(
            1e6, 1e6 + N).astype(numpy.int64), inplace=True)
        ...
        if len_df < 0:
            raise ValueError(
                "The number of matches is higher than the number of records N.")
        ...
        df_persons['entity_id'] = numpy.arange(
            max_entity_id + 1, max_entity_id + len_df + 1).astype(object)

        # Dataframe
        df_persons = df.sample(matches).append(
            pandas.DataFrame([fakeperson() for _ in range(0, len_df)]))
