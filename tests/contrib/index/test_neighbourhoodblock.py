...
from ...test_indexing import TestData
...
class TestNeighbourhoodBlock(TestData):
    ...
    def assert_index_comparisons(self, pairwise_comparison, indexers, *args, **kwargs):
        ...
        for index1, index2 in zip(indexes, indexes[1:]):
            ...
            assert (len(pairs1) == len(index1)) and (len(pairs2) == len(index2)) and pairwise_comparison(pairs1, pairs2)
    ...
    def test_dedup_single_blocking_key_vs_Block(self):
        indexers = [
            ...
            NeighbourhoodBlock(left_on='var_block10', right_on='var_block10', max_nulls=1),
            Block('var_block10'),
            ]
        self.assert_index_comparisons(eq, indexers, self.a)
    ...
    def test_link_single_blocking_key_vs_Block(self):
        indexers = [
            ...
            NeighbourhoodBlock(left_on='var_arange', right_on='var_arange', max_nulls=1),
            Block('var_arange'),
            ]
        self.assert_index_comparisons(eq, indexers, self.a, self.b)
        self.assert_index_comparisons(gt, indexers[-2:], self.incomplete_a, self.incomplete_b)

    def test_dedup_multiple_blocking_keys_vs_Block(self):
        indexers = [
            NeighbourhoodBlock(['var_single', 'var_block10'], max_nulls=1),
            NeighbourhoodBlock(left_on=['var_single', 'var_block10'], right_on=['var_single', 'var_block10'], max_nulls=1),
    ...
    def test_link_multiple_blocking_keys_vs_Block(self):
        indexers = [
            ...
            NeighbourhoodBlock(left_on=['var_arange', 'var_block10'], right_on=['var_arange', 'var_block10'], max_nulls=1),
    ...
    def test_dedup_single_sorting_key_vs_SortedNeighbourhood(self, window):
        indexers = [
            ...
            NeighbourhoodBlock(left_on='var_arange', right_on='var_arange', max_nulls=1, windows=window),
    ...
    def test_dedup_with_blocking_vs_SortedNeighbourhood(self, window):
        indexers = [
            NeighbourhoodBlock(['var_arange', 'var_block10'], max_nulls=1, windows=[window, 1]),
            NeighbourhoodBlock(left_on=['var_arange', 'var_block10'], right_on=['var_arange', 'var_block10'], max_nulls=1, windows=[window, 1]),
